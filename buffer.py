...
class OfflineReplayBuffer(OnlineReplayBuffer):
    ...
    def load_dataset(
        ...
        self._state = dataset['observations'][:-1, :] # (N-1, state_dim)
        self._action = dataset['actions'][:-1, :] # (N-1, action_dim)
        self._reward = dataset['rewards'].reshape(-1, 1)[:-1, :] # (N-1, 1)
        self._next_state = dataset['observations'][1:, :] # (N-1, state_dim)
        self._next_action = dataset['actions'][1:, :] # (N-1, action_dim)
